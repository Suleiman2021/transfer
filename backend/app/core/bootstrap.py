    ...
    if "transfers" in tables:
        with engine.connect() as conn:
            conn.execution_options(isolation_level="AUTOCOMMIT").execute(
                text("ALTER TYPE transfertype ADD VALUE IF NOT EXISTS 'customer_cashout'")
            )
        ...
        with engine.begin() as conn:
            ...
            conn.execute(
                text("ALTER TABLE transfers ADD COLUMN IF NOT EXISTS customer_name VARCHAR(120)")
            )
            conn.execute(
                text("ALTER TABLE transfers ADD COLUMN IF NOT EXISTS customer_phone VARCHAR(40)")
            )
            conn.execute(
                text(
                    "ALTER TABLE transfers "
...
def _feature_schema_mismatch() -> bool:
    ...
    return not required_commission_columns.issubset(commission_columns)
            ...
            if role not in existing_roles:
                db.add(
                    CommissionRule(
                        ...
                        agent_topup_profit_internal_percent=fees["agent_profit_internal"],
                        agent_topup_profit_external_percent=fees["agent_profit_external"],
